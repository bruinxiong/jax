...
def _odeint_rev(func, rtol, atol, mxstep, res, g):
  ...
  ts_bar = []
  t0_bar = 0.
  args_bar = tree_map(np.zeros_like, args)
  # TODO roll this into a loop, or perhaps combine with the loop over times
  # currently inside odeint
  for i in range(len(ts) - 1, 0, -1):
    ...
    y_bar = y_bar + g[i - 1]
  ts_bar = np.array([t0_bar] + ts_bar[::-1])
