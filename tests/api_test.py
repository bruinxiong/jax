...
class CustomJVPTest(jtu.JaxTestCase):
  ...
  def test_closed_over_tracers(self):
    raise unittest.SkipTest("TODO")  # TODO changed error message
    ...
    self.assertRaisesRegex(
        Exception,
        r"closed-over traced values: custom derivative rules can only be used "
        r"with top-level functions.",
        lambda: api.grad(f)(3.))
